...
import io.github.dubthree.mutantkiller.git.GitHubClient;
...
public class RunCommand implements Callable<Integer> {
    ...
    public Integer call() throws Exception {
        ...
        RepoInfo repoInfo = RepoInfo.parse(repoUrl);
        if (repoInfo == null) {
            System.err.println("Invalid repository URL: " + repoUrl);
            return 1;
        }

        System.out.println("=== Mutant Killer ===");
        System.out.println("Repository: " + repoInfo.owner() + "/" + repoInfo.name());
        System.out.println();

        // Set up working directory
        Path workPath = workDir != null 
            ? workDir.toPath() 
            : Path.of(System.getProperty("java.io.tmpdir"), "mutant-killer", repoInfo.name());
        
        // Initialize components
        RepositoryManager repoManager = new RepositoryManager(workPath, token);
        GitHubClient github = new GitHubClient(token, repoInfo.owner(), repoInfo.name());
        ...
        for (int i = 0; i < survived.size(); i++) {
            ...
            try {
                ...
                if (dryRun) {
                    ...
                    String prUrl = github.createPullRequest(branchName, baseBranch, prTitle, prBody);
    ...
    record RepoInfo(String owner, String name) {
        static RepoInfo parse(String url) {
            // Handle https://github.com/owner/repo or git@github.com:owner/repo.git
            String pattern1 = "github\\.com[/:]([^/]+)/([^/\\.]+)";
            var matcher = java.util.regex.Pattern.compile(pattern1).matcher(url);
            if (matcher.find()) {
